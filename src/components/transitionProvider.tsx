...
const TransitionProvider = ({ children }: any) => {
  ...
  return (
    <AnimatePresence mode="wait">
        <div key={pathName} className="w-screen h-full min-h-screen bg-gradient-to-b  from-purple-500 to-pink-500 -z-100">
            ...
            initial={{ opacity: 1 }}
            animate={{ opacity: 0 }} exit = {{ opacity: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
            ...
            <div className="h-[calc(100vh-6rem)]  mb-2">
              {children}
            </div>
          </div>
    </AnimatePresence>
